...
public:
	...
	: engine_(engine), input_(input), in_channel_(in_channel), layer_(layer), has_bias_(has_bias) {
		...
		int q_channel = NH * HS, kv_channel = NKV * HS;
		int total_channel = q_channel + 2 * kv_channel;
		weight_ = llm.parameter_storage({in_channel_, total_channel}, dtype_of<T>::value);
		llm.bind_parameter_slice(prefix + ".q_proj.weight", weight_, 0, q_channel, HS);
		llm.bind_parameter_slice(prefix + ".k_proj.weight", weight_, q_channel, kv_channel, HS);
		llm.bind_parameter_slice(prefix + ".v_proj.weight", weight_, q_channel + kv_channel, kv_channel);
		if (has_bias_) {
			bias_ = llm.parameter_storage({total_channel}, dtype_of<T>::value);
			llm.bind_parameter_slice(prefix + ".q_proj.bias", bias_, 0, q_channel, HS);
			llm.bind_parameter_slice(prefix + ".k_proj.bias", bias_, q_channel, kv_channel, HS);
			llm.bind_parameter_slice(prefix + ".v_proj.bias", bias_, q_channel + kv_channel, kv_channel);
		}
		out_ = llm.arena().alloc({max_tokens, q_channel}, dtype_of<T>::value);
